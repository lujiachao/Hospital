...
namespace BookUI
{
    public partial class FrmReportBillOutStat : Form
    {
        BillReportOut billReportOut = new BillReportOut();
        decimal _sto_ID;//获取登录仓库
        BillImageComBox billImageComBox = new BillImageComBox(); //messagebox类
        #region 打印公共元素
        DataTable dt1;
        //以下用户可自定义
        //当前要打印文本的字体及字号
        ...
        #endregion
        public FrmReportBillOutStat(decimal Sto_id)
        {
            InitializeComponent();
            _sto_ID = Sto_id;
        ...
        //load函数
        private void FrmReportBillOutStat_Load(object sender, EventArgs e)
        {
        ...
        //出库分类改变事件
        private void imageComboBoxEditInType_EditValueChanged(object sender, EventArgs e)
        {
            decimal id = Convert.ToDecimal(imageComboBoxEditInType.EditValue);
            char target = billReportOut.getTarget(id);
            if (target == '1')
            {
        ...
        private void simpleButtonSearch_Click(object sender, EventArgs e)
        {
            ...
            decimal? boutType = Convert.ToDecimal(imageComboBoxEditInType.EditValue);
            char targetType = Convert.ToChar(imageComboBoxEditSearchTargetType.EditValue);
            decimal target_ID = Convert.ToDecimal(imageComboBoxEdit1.EditValue);
            labelControlDateTimeShow.Text = "年月范围：" + dateEditTimeStart.Text + "到" + dateEditTimeEnd.Text;
            DataTable dt = billReportOut.GetCheckedBillOutMainListOutCommShow(_sto_ID,acc_id,boutType,target_ID,dateStart,dateEnd,searchState);
        ...
        private void gridView1_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (dr != null)
            {
                ...
                labelControlCreateEmp.Text = "制单人： " + dr["INPUT_EMP"].ToString();
                labelControlCreateDate.Text = "制单日期： " + dr["INPUT_TIME"].ToString();
            }
        }
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            if (PrintingPageNumber == 1 || PageRecordNumber > TotalNum)//最后一页只打印统计行时不打印列名
            {
                ...
                for (int Cols = 0; Cols <= DataTablePrint.Columns.Count - 1; Cols++)
                {
                    ...
                    else if (ColumnText == "TARGET_NAME")
                    {
                        ColumnName = "出库单位";
                    }
                    ...
                    else if (ColumnText == "BUYING_RETAIL_SUB")
                    {
                        ColumnName = "进销差额";
                    }
                    else
